...
链路 (v2.0 五步):
    Query → BGE-M3 Embedding → Qdrant 检索 → Reranker 重排序 → LLM 推理
...
class RAGPipeline:
    ...
    def __init__(self) -> None:
        ...
        logger.info(
            f"RAGPipeline v2.0 initialized | "
    ...
    async def ainvoke(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        top_k = inputs.get("top_k", settings.RETRIEVAL_K)
        rerank_top_k = inputs.get("rerank_top_k", settings.RERANK_TOP_K)

        if not query.strip():
            return {"input": query, "context": [], "output": "请输入有效问题。"}

        logger.info(f"RAG query | len={len(query)} | retrieval_k={top_k} | rerank_top_k={rerank_top_k}")
        ...
        try:
            ...
            if not hits:
                return {
                    "input": query,
                    "context": [],
                    "output": "⚠️ 未在知识库中检索到相关内容。请先导入文档。",
                }
            logger.info(f"Retrieved {len(hits)} hits | top_score={hits[0].score:.4f}")

            # ── 3. Reranker 重排序 ──────────────────
            try:
                reranked = self.reranker.rerank(query, hits, top_k=rerank_top_k)
                logger.info(f"Reranking done | {len(reranked)} docs")
                ...
                logger.warning(f"Reranker failed: {e}, falling back to raw search results.")
                # 降级处理：直接用第一步搜索回来的 Hits
                context_snippets = [h.text for h in hits[:rerank_top_k]]
                context = "\n\n---\n\n".join(
                    f"[原始检索结果 {i}] (得分: {getattr(h, 'score', None) or 'N/A'})\n{h.text}"
                    for i, h in enumerate(hits[:rerank_top_k], 1)
                )
